...
class UserCardServiceTest {
  ...
  @Mock
  RestTemplate restTemplate; // ✅ 여기 선언 가능
  ...
  void getPaymentHistory_sortWithoutDirection_defaultsToDesc() {
    ...
    // when
    Page<PaymentHistoryResponse> result = userCardService.getPaymentHistory(
            userId, page, size, sort, null, null);
  ...
  void convertToBenefitResponses_bothMerchantsNull() {
    ...
    List<BenefitResponse> result = userCardService.convertToBenefitResponses(card, MerchantCategory.SHOPPING);
  ...
  void convertToBenefitResponses_b1Null_b2DifferentCategory() {
    ...
    Card card = Card.builder().cardBenefits(List.of(
            CardBenefit.builder().benefit(b1).build(),
            CardBenefit.builder().benefit(b2).build()
    )).build();

    List<BenefitResponse> result = userCardService.convertToBenefitResponses(card, MerchantCategory.FOOD_BEVERAGE);
  ...
  void convertToBenefitResponses_bothMatchesTrue_triggersComparison() {
    ...
    Benefit b1 = Benefit.builder()
            ...
            .merchant(Merchant.builder()
                    .id(1L)
                    .category(target) // ✅ 일치
                    .isActive(true).isDeleted(false).commissionRate("0%")
                    .businessNumber("000").name("A").managerName("홍").phone("010").managerPhone("011")
                    .build())
            .benefitType(BenefitType.DISCOUNT)
            .hasAdditionalCondition(false)
            .benefitConditions(List.of())
    ...
    Benefit b2 = Benefit.builder()
            ...
            .merchant(Merchant.builder()
                    ...
                    .isActive(true).isDeleted(false).commissionRate("0%")
                    .businessNumber("001").name("B").managerName("김").phone("010").managerPhone("012")
                    .build())
            .benefitType(BenefitType.DISCOUNT)
            .hasAdditionalCondition(false)
            .benefitConditions(List.of())
  ...
  void getPaymentHistory_withStartAndEndDate() {
    ...
    given(paymentRepository.findByUserCard_User_IdAndCreatedAtBetween(userId, startDate, endDate, pageable))
            .willReturn(Page.empty());

    // when
    Page<PaymentHistoryResponse> result = userCardService.getPaymentHistory(
            userId, page, size, sort, startDate, endDate);
  ...
  void addCard_unexpectedStatus_throwsUncaught() {
    ...
    given(userRepository.findById(userId)).willReturn(Optional.of(User.builder().userCards(List.of()).build()));
  ...
  void addCard_success() {
    ...
    AddCardRequest request = AddCardRequest.builder()
            .cardNumber("1111222233334444").month("12").year("26")
            .cvc("123").pinPrefix("00").build();

    User user = User.builder()
            .id(userId).name("홍길동").birthDate("921010").phone("01012345678")
            .userCards(List.of()) // 첫 카드
            .build();

    CardTokenResponse tokenResponse = new CardTokenResponse( 77L, "token123");
    CommonResponse<CardTokenResponse> commonResponse = new CommonResponse<>(true, HttpStatus.OK, "", tokenResponse);
  ...
  void addCard_alreadyExistsToken_throws() {
    ...
    AddCardRequest request = AddCardRequest.builder().cardNumber("1111").build();
    CardTokenResponse tokenResponse = new CardTokenResponse(77L,"token123");
    CommonResponse<CardTokenResponse> response = new CommonResponse<>(true, HttpStatus.OK, "", tokenResponse);
  ...
  void getCardDetail_success() {
    ...
    assertThat(benefitRes.getBenefitConditions().get(0).getCategory()).isEqualTo("MONTHLY_DISCOUNT_LIMIT");
  }
  ...
  void getCardRecommendation_success() {
    ...
    Benefit matching = Benefit.builder()
            .id(1L)
            .title("외식 할인")
            .description("5% 할인")
            .benefitType(BenefitType.DISCOUNT)
    ...
    Benefit nonMatching = Benefit.builder()
            .id(2L)
            .title("쇼핑 할인")
            .description("3% 할인")
            .benefitType(BenefitType.DISCOUNT)
