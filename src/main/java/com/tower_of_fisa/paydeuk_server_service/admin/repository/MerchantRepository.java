...
public interface MerchantRepository extends JpaRepository<Merchant, Long> {
  ...
  @Query(
      "SELECT m, COUNT(p.id) AS transactionCount, COALESCE(SUM(p.amount), 0) AS totalAmount "
          ...
          + "WHERE (:status IS NULL OR m.isActive = CASE WHEN :status = 'ACTIVE' THEN true WHEN :status = 'INACTIVE' THEN false ELSE m.isActive END) "
          + "AND (:search IS NULL OR LOWER(m.name) LIKE LOWER(CONCAT('%', :search, '%'))) "
          + "GROUP BY m.id "
          + "ORDER BY "
          + "  CASE WHEN :sort = '거래건수순' THEN COUNT(p.id) END DESC, "
          + "  CASE WHEN :sort = '금액순' THEN SUM(p.amount) END DESC, "
