...
/**
 * UserStatus Enum과 데이터베이스 문자열 간의 변환을 처리하는 컨버터
 * 이 컨버터는 다음과 같은 기능을 제공합니다:
 * 1. 데이터베이스에 저장 시: UserStatus enum의 name() 값을 그대로 저장
 * 2. 데이터베이스에서 조회 시: 문자열을 대문자로 변환하여 UserStatus enum으로 매핑
 * 
 * 예시:
 * - DB의 'active' -> UserStatus.ACTIVE
 * - DB의 'ACTIVE' -> UserStatus.ACTIVE
 * 
 * 주의사항:
 * - 데이터베이스의 값이 enum에 정의되지 않은 경우 null을 반환
 */
