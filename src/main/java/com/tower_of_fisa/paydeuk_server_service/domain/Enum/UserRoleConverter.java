...
/**
 * UserRole Enum과 데이터베이스 문자열 간의 변환을 처리하는 컨버터
 * 이 컨버터는 다음과 같은 기능을 제공합니다:
 * 1. 데이터베이스에 저장 시: UserRole enum의 name() 값을 그대로 저장
 * 2. 데이터베이스에서 조회 시: 문자열을 대문자로 변환하여 UserRole enum으로 매핑
 * 예시:
 * - DB의 'user' -> UserRole.USER
 * - DB의 'USER' -> UserRole.USER
 */
